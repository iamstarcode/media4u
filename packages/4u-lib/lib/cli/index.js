...
export const checkFormat = (episodes) => {
    for (let i = 0; i < episodes.selectedEpisodes; i++) {
        const episode = episodes[i];
        const regexPattern = /^s[1-9][0-9]*:([1-9][0-9]*)-?([1-9][0-9]*)?(?:,([1-9][0-9]*)-?([1-9][0-9]*)?)*$/;
        if (!regexPattern.test(episode)) {
            ...
            console.log(chalk.yellow('s[season_number]:[episode_number-episode_number][...]'));
...
export const handleEpisodes = (selectedEpisodes) => {
    ...
    for (let i = 0; i < selectedEpisodes.length; i++) {
        ...
        const streamedEpisodes = episodesSeperated(ep.substring(ep.indexOf(':') + 1));
        eps.push({ season: parseInt(season), episodes: streamedEpisodes });
    }
    return eps;
};
export const episodesSeperated = (value) => {
    ...
    return _.sortBy(unique, (o) => o);
};
export const handleIfNewVersion = async (version, pkgName) => {
